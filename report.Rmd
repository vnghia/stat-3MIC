...
```
```{r pop_class_table, fig.cap = "Diagramme en barre de popularité", echo = FALSE}
...
```
Du fait que son kurtosis est inférieur à $-1,2$ (le kurtosis de la distribution uniforme[^3]),
sa distribution aura la forme d'une vallée (car la distribution uniforme est déjà une ligne).

```{r daf_acousticness_boxplot, fig.cap = "Histogramme d'acoustique des chansons", echo = FALSE}
hist(daf$acousticness, main=NULL, ylab="Fréquence", xlab="Acoustique", col="cadetblue1")
```

[^3]: https://en.wikipedia.org/wiki/Kurtosis#Other_well-known_distributions
...
```{r daf_acousticness_popclass_boxplot, fig.cap = "Boxplot parallèles de la relation entre acoustique et popularité", echo = FALSE}
